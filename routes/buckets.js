...
router.get('/', function (req, res) {
    connection.query('SELECT * FROM buckets', function (err, rows) {
        if (err) {
...
router.post('/', function (req, res) {
    const { name } = req.body;
    connection.query(`INSERT INTO buckets
    (name)
    VALUES('${name}')
    `, function (err, rows) {
        if (err) {
        ...
        if(rows.affectedRows) {
            return res.status(200).send({
                status: 'success',
                data: 'Bucket Created successfully.'
            });
        }
    });
});
        ...
        if(isEmpty(rows)) {
            ...
            connection.query(url, function (err1, rows1) {
                ...
                if(rows1.affectedRows) {
                    return res.status(200).send({
                        status: 'success',
                        data: 'Bucket updated successfully.'
                    });
                }
            });
        }
...
router.get('/:id/tasks', function (req, res) {
    const { id } = req.params;
    connection.query(`SELECT * FROM tasks WHERE bucketId = ${id}`, function (err, rows) {
        if (err) {
            return res.status(400).send({
                status: 'Failure',
                data: err.code
